...
// Uniswap V2 Factory — official source with pragma bumped to 0.8.24
...
contract UniswapV2Pair {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint rootK = sqrt(uint(_reserve0) * uint(_reserve1));
    ...
    // UQ112x112 helpers
    function encode(uint112 y) internal pure returns (uint224) { return uint224(y) * 2**112; }
